<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="success" plain icon="Plus" @click="handleAddOrUpdate()" v-hasPermi="['system:dept:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    >
      ...
      <el-table-column prop="nickName" label="负责人">
        <template #default="scope"> <employee-card v-model="scope.row.userId" :label="scope.row.nickName" /></template>
      </el-table-column>
      <el-table-column label="创建时间" align="center" prop="createTime" width="200" />
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleAddOrUpdate(scope.row.id)" v-hasPermi="['system:dept:edit']">修改</el-button>
          <el-button link type="success" icon="Plus" @click="handleAddOrUpdate(null, scope.row.id)" v-hasPermi="['system:dept:add']">新增</el-button>
          <el-button
...
<script setup name="Dept">
  ...
  function getDataList() {
    loading.value = true;
    deptApi.query(queryParams.value).then((res) => {
      dataList.value = proxy.handleTree(res.records, 'id');
      loading.value = false;
    });
  }
